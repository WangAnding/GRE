...
public:
    ...
    //----for SOSD benchmarkBuild 
    //函数用于构建 DILI 数据结构，它接受一个包含键值对的向量 data 作为参数，并返回一个 uint64_t 类型的值。
    //这个函数会调用 bulk_load 函数来执行构建操作，并使用 linux_sys_utils::timing 函数来计算构建时间。
    ...
    //EqualityLookup 函数用于在 DILI 数据结构中进行等值查找。
    //接受一个 lookup_key 作为参数，并返回一个 SearchBound 结构。
    //这个函数会调用 search 函数来执行查找操作，并返回查找结果的起始位置和结束位置。
